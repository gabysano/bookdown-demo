...
```{r}
cdwr_flow_data <- function(site, start_wyear, end_wyear){
  
  # First pull the url with the parameters of interest
  raw_data <- GET(url = paste0(
    "https://dwr.state.co.us/Rest/GET/api/v2/surfacewater/surfacewatertsday/?format=json&dateFormat=dateOnly&fields=abbrev%2CmeasDate%2Cvalue%2CmeasUnit&encoding=deflate&abbrev=", site,
  ...
  # returns only the time series flow data we are interested in and binds them together since they are their own list
  return(bind_rows(final_data[5]))
}
```
...
cdwr <- sites %>%
  map(~ cdwr_flow_data(site = ., start_wyear = 2021, end_wyear = 2022)) %>%
  bind_rows()

# Clean up cdwr column names so they match the column names from usgs data frame
colnames(cdwr) <- c("Site", "Date", "q_cfs")

# removes the unit column
cdwr <- cdwr[,-4]

# makes the Date column a date field in the same format as the date field in the usgs data frame
cdwr$Date %>% as.Date(.,"%Y-%m-%d") -> cdwr$Date
```
...
```{r}
# first bind both data frames
total_flow_data <- bind_rows(usgs, cdwr)

# interactive plot
plotly::ggplotly(
  ggplot(data = total_flow_data) +
    geom_point(aes(x = Date, y = q_cfs, color = Site)) +
    geom_path(aes(x = Date, y = q_cfs, color = Site)) +
    scale_y_discrete(labels = scales::label_scientific()) +
...
```{r}
# CLAFTCCO is Canyon Mouth

# Make our narrow data into wide data
wide_flow_data <- total_flow_data %>%
  select(Date, q_cfs, Site) %>%
  pivot_wider(., names_from = Site, 
              values_from = q_cfs)

# selects all of the downstream sites 
downstream_sites <- wide_flow_data %>%
  select(-c("Date", "CLAFTCCO"))

# creates a table of the difference in discharge between the canyon mouth and each of the downstream sites
diff_table <- wide_flow_data %>%
  mutate(CLAFTCCO - downstream_sites) %>%
  pivot_longer(cols = -Date,
               names_to = "Site",
               values_to = "q_Difference_cfs")

# plot of the daily difference in discharge between canyon mouth and downstream sites
plotly::ggplotly(
  ggplot(data = diff_table, (aes(x = Date, y = q_Difference_cfs, color = Site))) +
    geom_point() +
    geom_path() +
    scale_y_discrete(labels = scales::label_scientific()) +
...
```{r}
Lincoln_highq <- diff_table %>%
  filter(Site == "Lincoln" & q_Difference_cfs < 0)

paste0("Canyon mouth had lower flow than the Lincoln Bridge downstream site for ", count(Lincoln_highq), " days.")

Boxelder_highq <- diff_table %>%
  filter(Site == "Boxelder" & q_Difference_cfs < 0)

paste0("Canyon mouth had lower flow than the Environmental Learning Center downstream site for ", count(Boxelder_highq), " days.")

CLARIVCO_highq <- diff_table %>%
  filter(Site == "CLARIVCO" & q_Difference_cfs < 0)

paste0("Canyon mouth had lower flow than the Below Fossil Creek Reservoir downstream site for ", count(CLARIVCO_highq), " days.")
```
